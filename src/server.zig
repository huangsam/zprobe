...
fn workerMain(ctx: WorkerContext) void {
    while (true) {
        ...
        handleConnection(ctx.allocator, ctx.io, conn, ctx.database, ctx.auth_user, ctx.auth_pass) catch {};
    }
}
...
fn handleRequest(
    ...
    if (auth_user != null and auth_pass != null) {
        if (!isRequestAuthorized(request, auth_user.?, auth_pass.?)) {
...
fn isHeaderIteratorAuthorized(it: anytype, auth_user: []const u8, auth_pass: []const u8) bool {
    var iter = it;
...
fn isRequestAuthorized(request: *const std.http.Server.Request, auth_user: []const u8, auth_pass: []const u8) bool {
