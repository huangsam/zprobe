...
13. **Class-Based CSS over ID Selectors**: CSS rules should target classes (e.g. `.search-input`) rather than ID selectors (e.g. `#search-input`) to keep specificity low and maintain scalability. However, HTML elements must retain their `id` attributes so that JavaScript DOM queries (`document.getElementById`) remain unbroken. The only exceptions are `#catalog-table` and its column headers (`#th-path`, etc.), which must continue to use ID selectors in CSS due to table-layout performance constraints (see Rule 10).
