...
11. **Content-Keyed Preview Generation & Aligned Formats**: Previews and thumbnails are keyed by the media content hash (`media_metadata.file_hash` / `computeFastHash`, 64-hex lowercase) — not by path. The same content at multiple paths shares one on-disk JPEG poster (`.jpg`) and animated GIF preview (`.gif`, built with ffmpeg's core gif encoder via a palettegen/paletteuse pass — no external library required). Layout: `.zprobe_thumbnails/<aa>/<bb>/<file_hash>.jpg` and `.zprobe_animations/<aa>/<bb>/<file_hash>.gif` (two-level hex shard); writers always `createDirPath` the parent before writing. Never write under path-hash or synthetic `{size}_{mtime}_…` stems; never claim `has_thumbnail` / `has_animated` without a `stat` of the content-keyed artifact. The `/api/thumbnail?path=` API keeps the path query param and resolves path → `file_hash` → disk. The definition of what constitutes a video is strictly unified between the crawler scanner extensions, the SQL database query predicates, and the frontend JS router (`mp4, m4v, webm, mkv, mov, avi, wmv, flv`).
12. **Lock-Free Atomic Performance Profiling**: Performance and duration metrics are captured thread-safely using lock-free atomic counters (`std.atomic.Value(u64)`) to prevent lock contention between worker threads. Monotonic duration timings utilize the system's uptime clock (`.awake`) via a custom stack-allocated/zero-heap `MonotonicTimer`. All timing hooks verify if profiling is enabled first, guaranteeing exactly zero runtime overhead when `--profile` is not requested.
13. **Class-Based CSS over ID Selectors**: CSS rules should target classes (e.g. `.search-input`) rather than ID selectors (e.g. `#search-input`) to keep specificity low and maintain scalability. However, HTML elements must retain their `id` attributes so that JavaScript DOM queries (`document.getElementById`) remain unbroken. The only exceptions are the table column headers (`#th-path`, etc.), which must continue to use ID selectors in CSS due to table-layout performance constraints (see Rule 10).
14. **CSS Design Token Discipline**: All colors, radii, spacing, transitions, and z-indices must reference established design tokens declared in `src/web/css/variables.css`. Avoid hardcoded hex/named colors (use `var(--white)`, `var(--text-muted)`) or raw magic radii (use `var(--radius-xs)`, `var(--radius-sm)`, `var(--radius-md)`, `var(--radius-lg)`, `var(--radius-full)`). When introducing a new token, declare it in `variables.css` first, ensuring referenced variables always exist.
15. **Compile-Time Asset Registration**: The web frontend operates with zero runtime bundlers, concatenating CSS and JS fragments at compile time directly in `src/web/assets.zig`. When creating new CSS or JS modules, they *must* be explicitly registered in `styles_css` or `app_js` in `src/web/assets.zig` in the correct dependency order. Conversely, when removing or renaming a file, update `assets.zig` immediately to prevent stale builds or missing assets.
16. **Modal & Overlay Event Ownership**: Modal-specific keydown handlers (Escape key closing, Tab focus trapping) and backdrop click listeners must be owned and dynamically bound/unbound by that modal’s lifecycle controller function (e.g. `toggleFilterModal`, `toggleSortModal`, `toggleModal`), rather than added unconditionally to global window listeners. Global listeners in `main.js` must not duplicate or conflict with component-level listeners.
