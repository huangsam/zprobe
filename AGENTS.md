...
11. **Path-Based Preview Generation & Aligned Formats**: Previews and thumbnails are derived via path-based hashing: `sha256(original_path)` determines both the JPEG poster (`.jpg`) and the animated GIF preview (`.gif`, built with ffmpeg's core gif encoder via a palettegen/paletteuse pass - no external library required). These live under `.zprobe_thumbnails` using a two-level hex shard (`aa/bb/<hash>.{jpg,gif}`) for directory scaling; writers always `createDirPath` the parent before writing. The definition of what constitutes a video is strictly unified between the crawler scanner extensions, the SQL database query predicates, and the frontend JS router (`mp4, m4v, webm, mkv, mov, avi, wmv, flv`).
